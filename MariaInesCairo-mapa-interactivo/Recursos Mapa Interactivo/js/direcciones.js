direccionesModulo = (function () {
  ...
  function calcularYMostrarRutas () {
        ...
        /* Completar la función calcularYMostrarRutas , que dependiendo de la forma en que el
         ...
         let medioTransporte = document.getElementById('comoIr').value;
         let puntosIntermedios = [];
         let puntosIntHtml = document.getElementById('puntosIntermedios');

        for (var i = 0; i < puntosIntHtml.length; i++) {
          if (puntosIntHtml.options[i].selected) {
            puntosIntermedios.push({
              location: puntosIntHtml[i].value,
              stopover: true
            })
          }
        }
